  ...
  while IFS= read -r -d '' f; do
    ...
    case " $ALLOWED " in
      ...
      *)
        ...
        if [[ "$REL" != "SKILL.md" && "$REL" != ".pinned" && "$REL" != ".agent-created" && "$REL" != ".agent-created.json" && "$REL" != ".promotion-reviewed.json" && "$REL" != ".skill-evaluation-cases.json" && "$REL" != ".skill-evaluation-policy.json" ]]; then
