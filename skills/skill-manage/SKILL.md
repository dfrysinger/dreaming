...
> **Paths note:** Run this skill's helpers from
> `$DREAMING_REPO_ROOT/skills/skill-manage`. `$SKILLS_REPO_ROOT` is the
> optional public catalog; `$SKILLS_LOCAL_ROOT` and `$SKILLS_STATE_DIR` retain
...
- **Editing an installed cache or immutable bundle.** Always operate under
  `~/code/skills/` (PUBLIC) or `~/.copilot/skills/` (LOCAL). Never edit
  `~/.copilot/installed-plugins/` or `$DREAMING_SHARED_SKILLS_ROOT`.
