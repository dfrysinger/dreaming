...
> (this repository). Shared dependencies resolve only from
> `$DREAMING_SHARED_SKILLS_ROOT`. `$SKILLS_REPO_ROOT` is the optional public
> catalog; `$SKILLS_LOCAL_ROOT` and `$SKILLS_STATE_DIR` retain their existing
> meanings.
