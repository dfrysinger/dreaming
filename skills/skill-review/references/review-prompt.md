...
`~/.hermes/skills/` → the managed public and local Copilot skill roots, the `skill_manage` /
...
>   • Bundled skills shipped by other plugins. Only
>     `$SKILLS_REPO_ROOT/skills/` and `$SKILLS_LOCAL_ROOT/` are writable
>     targets. Dreaming-owned and verified shared skills are read-only
>     dependencies.
...
9. **Guards:** before any action, record the starting commit of each root and
   ...
   guards. (a)
   `verify-repo-unchanged.sh check` — `$SKILLS_REPO_ROOT` must
