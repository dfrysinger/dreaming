...
TASK_OPPORTUNITY_TAXONOMY_ID = (
    "sha256:2f8953ce561e0ffab895ce2d7a272ada7f7b56347314d9becfed285e946c008a"
)
TASK_OPPORTUNITY_CLASSIFIER_ID = (
    "sha256:1a098f3ff8187c8d8dc21dec64dfea1e1a1bb36f14d2eb8207224b1fca8187d8"
)
TASK_OPPORTUNITY_CATEGORY_IDS = {
    "3d-printing",
    "canvas-extension",
    "cloud-agent-customization",
    "diagramming",
    "entitlements",
    "expense-report",
    "issue-kanban",
...
MAX_TASK_OPPORTUNITY_RECEIPT_EPISODES = 100_000
...
def validate_estate_opportunity_snapshot(
    ...
    snapshot_sha256 = opportunity.get("snapshot_sha256")
    snapshot = {
        key: value
    ...
    if (
        set(opportunity) != expected_keys
        or type(opportunity.get("schema_version")) is not int
        or opportunity.get("schema_version") != 1
        or opportunity.get("kind") != "task_opportunity_snapshot"
        or not isinstance(snapshot_sha256, str)
        or digest(snapshot) != snapshot_sha256
        or opportunity.get("census_snapshot_sha256")
        != census.get("snapshot_sha256")
        or opportunity.get("source_usage_snapshot_sha256")
        != usage.get("snapshot_sha256")
        or opportunity.get("host_id") != census.get("host_id")
        or opportunity.get("collected_at") != census.get("collected_at")
        or opportunity.get("taxonomy_id") != TASK_OPPORTUNITY_TAXONOMY_ID
        or opportunity.get("classifier_id")
        != TASK_OPPORTUNITY_CLASSIFIER_ID
        or opportunity.get("collection_watermark")
        != usage.get("coverage", {}).get("collection_watermark")
    ):
        raise RuntimeFailure("estate-opportunity-invalid", "snapshot contract")
    collected_at = parse_time(opportunity.get("collected_at"))
    window_start = parse_time(opportunity.get("window_start"))
    window_end = parse_time(opportunity.get("window_end"))
    if (
        ...
        raise RuntimeFailure("estate-opportunity-invalid", "decision window")
    counts = [
        opportunity.get("stable_episode_count"),
        opportunity.get("classified_episode_count"),
        opportunity.get("unclassified_episode_count"),
    ]
    if (
        any(
        ...
        or counts[0] > MAX_TASK_OPPORTUNITY_RECEIPT_EPISODES
        or counts[1] + counts[2] != counts[0]
    ):
        raise RuntimeFailure("estate-opportunity-invalid", "episode totals")

    usage_coverage = usage.get("coverage")
    coverage = opportunity.get("coverage")
    excluded = opportunity.get("excluded_recent")
    backlog = opportunity.get("relevant_stable_backlog")
    if (
        ...
        raise RuntimeFailure("estate-opportunity-invalid", "coverage shape")
    bounded_values = (
        excluded["count"],
        excluded["bytes"],
        backlog["count"],
    ...
    if any(
        ...
        raise RuntimeFailure("estate-opportunity-invalid", "coverage totals")
    if (
        ...
        raise RuntimeFailure("estate-opportunity-invalid", "coverage binding")

    failure_ids = opportunity.get("failure_ids")
    ...
    if (
        ...
        raise RuntimeFailure("estate-opportunity-invalid", "failure binding")

    categories = opportunity.get("categories")
    if (
        not isinstance(categories, list)
        or len(categories) > len(TASK_OPPORTUNITY_CATEGORY_IDS)
    ):
        raise RuntimeFailure("estate-opportunity-invalid", "category shape")
    ...
    for category in categories:
        if (
            ...
            not in TASK_OPPORTUNITY_CATEGORY_IDS
            or category["category_id"] in seen_categories
        ):
            raise RuntimeFailure("estate-opportunity-invalid", "category identity")
        seen_categories.add(category["category_id"])
        numeric = [
            category.get("episode_count"),
            category.get("distinct_session_count"),
            category.get("distinct_day_count"),
            category.get("completed_count"),
        ]
        observed_at = parse_time(category.get("last_observed_at"))
        if (
            any(
                not isinstance(value, int)
                or isinstance(value, bool)
            ...
            raise RuntimeFailure("estate-opportunity-invalid", "category totals")
        category_total += numeric[0]
    if category_total != counts[1]:
        raise RuntimeFailure("estate-opportunity-invalid", "classified total")

    uncertainty = opportunity.get("uncertainty")
    if not isinstance(uncertainty, list) or len(uncertainty) != counts[2]:
        raise RuntimeFailure("estate-opportunity-invalid", "uncertainty total")
    ...
    for episode in uncertainty:
        if (
            ...
            or re.fullmatch(
                r"sha256:[0-9a-f]{64}", episode.get("episode_id")
            )
            is None
            ...
            or any(
                category_id not in TASK_OPPORTUNITY_CATEGORY_IDS
                for category_id in episode["possible_category_ids"]
            )
            ...
            raise RuntimeFailure(
                "estate-opportunity-invalid", "uncertainty projection"
            )
        seen_episodes.add(episode["episode_id"])
    return collected_at, coverage
...
class RuntimePaths:
    ...
    @property
    def estate_opportunity_current(self) -> Path:
        return self.state / "estate-opportunity-current.json"

    @property
    def estate_opportunity_receipts(self) -> Path:
        return self.state / "estate-opportunity-receipts"
...
class DreamingRuntime:
    ...
    def record_estate_opportunity(
        ...
        collected_at, coverage = validate_estate_opportunity_snapshot(
            opportunity,
            census,
            usage,
        )
        ...
        if (
            ...
            or not re.fullmatch(
                r"sha256:[0-9a-f]{64}", usage_receipt_sha256
            )
        ):
            raise RuntimeFailure("estate-opportunity-invalid", "receiver identity")
        receipt = {
            "schema_version": 1,
            "snapshot_sha256": snapshot_sha256,
            "census_snapshot_sha256": census["snapshot_sha256"],
            "source_usage_receipt_sha256": usage_receipt_sha256,
            "receiver": {
                key: receiver[key] for key in sorted(required_receiver)
            },
        ...
        receipt_sha256 = digest(receipt)
        existing = read_json(self.paths.estate_opportunity_current, {})
        existing_opportunity = (
            existing.get("opportunity") if isinstance(existing, dict) else None
        )
        existing_collected_at = (
            parse_time(existing_opportunity.get("collected_at"))
            if isinstance(existing_opportunity, dict)
            else None
        )
        if existing_collected_at is not None and existing_collected_at.tzinfo is None:
            raise RuntimeFailure(
                "estate-opportunity-current-invalid",
                "collection time",
            )
        if (
            existing_collected_at == collected_at
            and existing.get("receipt_sha256") != receipt_sha256
        ):
            raise RuntimeFailure(
                "estate-opportunity-current-conflict",
        ...
        receipt_path = (
            self.paths.estate_opportunity_receipts
            / f"{receipt_sha256.removeprefix('sha256:')}.json"
        )
        if receipt_path.exists():
            if read_json(receipt_path, {}) != receipt:
                raise RuntimeFailure(
                    "estate-opportunity-receipt-collision", receipt_sha256
                )
        else:
            atomic_json(receipt_path, receipt, mode=0o600)
        current = {
        ...
        if is_current:
            atomic_json(
                self.paths.estate_opportunity_current,
                current,
                mode=0o600,
            )
        return {
            ...
            "snapshot_sha256": snapshot_sha256,
            "complete": coverage.get("complete") is True,
            "stable_episode_count": opportunity.get("stable_episode_count"),
            "classified_episode_count": opportunity.get(
                "classified_episode_count"
            ),
        }
...
def collect_estate_census(
    ...
    *,
    ...
    if opportunity is not None:
        if usage is None or not isinstance(opportunity, dict):
            raise RuntimeFailure(
                "estate-census-malformed", "opportunity malformed"
            )
        recorded["opportunity"] = core.record_estate_opportunity(
            opportunity,
            receiver,
            census,
            usage,
            recorded["usage"]["receipt_sha256"],
        )
    else:
        recorded["opportunity"] = {
            "status": "unavailable",
            "reason": "collector_generation_has_no_opportunity",
        }
    if include_evidence:
        return {
            "summary": recorded,
