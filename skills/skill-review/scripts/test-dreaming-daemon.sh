...
mkdir -p "$TEST_ROOT"
TMP="$(mktemp -d "$TEST_ROOT/dreaming-daemon.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
export TMPDIR="$TMP"
...
export DREAMING_RECEIPT_FILE="$TMP/shared-receipt.json"
"$REPO/scripts/dreaming-deps.py" generate-receipt "$DREAMING_SHARED_SKILLS_ROOT" \
