...
SCRATCH_ROOT="${DREAMING_SCRATCH_DIR:-${SKILLS_STATE_DIR:-$HOME/.copilot/skill-state}/dreaming/scratch}"
mkdir -p "$SCRATCH_ROOT"
RUN_DIR="$(mktemp -d "$SCRATCH_ROOT/skill-evaluation.XXXXXX")"
PLUGIN_DIR="$(mktemp -d "$SCRATCH_ROOT/skill-evaluation-plugin.XXXXXX")"
SANDBOX_DIR="$(mktemp -d "$SCRATCH_ROOT/skill-evaluation-sandbox.XXXXXX")"
