...
class RuntimeTest(unittest.TestCase):
    ...
    def test_estate_usage_receipt_is_separate_and_census_bound(self) -> None:
        ...
        usage_snapshot = {
            ...
            "coverage": {
                ...
                "collection_watermark": census["collected_at"],
                "pending_sessions": 0,
                "pending_bytes": 0,
                "bound_reached": None,
                "failures": [],
    ...
    def test_estate_opportunity_receipt_binds_usage_and_census(self) -> None:
        core = self.core({("fake", "executor")})
        census_snapshot = {
            "schema_version": 1,
            "host_id": "macbook",
        ...
        opportunity_snapshot = {
            ...
            "taxonomy_id": runtime_module.TASK_OPPORTUNITY_TAXONOMY_ID,
            "classifier_id": runtime_module.TASK_OPPORTUNITY_CLASSIFIER_ID,
            "collection_watermark": census["collected_at"],
            "window_start": "2026-07-18T18:00:00+00:00",
            "window_end": census["collected_at"],
            ...
            "categories": [
                {
                    ...
                    "completed_count": 1,
                    "last_observed_at": census["collected_at"],
                }
            ],
            "coverage": {
                "complete": True,
                "source_usage_complete": True,
                "corpus_complete": True,
                "pending_sessions": 0,
                "pending_bytes": 0,
                "bound_reached": None,
        ...
        opportunity = {
            **opportunity_snapshot,
            "snapshot_sha256": runtime_module.digest(opportunity_snapshot),
        }
        receiver = {
            "receiver_id": "fixture",
            "receiver_sha256": "a" * 64,
        ...
        core.record_estate_census(census, receiver)
        usage_result = core.record_estate_usage(usage, receiver, census)
        first = core.record_estate_opportunity(
            opportunity,
            receiver,
            census,
            usage,
            usage_result["receipt_sha256"],
        )
        second = core.record_estate_opportunity(
            opportunity,
            receiver,
            census,
            usage,
        ...
        self.assertEqual(first, second)
        same_time = copy.deepcopy(opportunity)
        same_time["categories"][0]["completed_count"] = 0
        same_time["snapshot_sha256"] = runtime_module.digest(
            {
                key: value
                for key, value in same_time.items()
                if key != "snapshot_sha256"
            }
        )
        with self.assertRaisesRegex(
            ...
            core.record_estate_opportunity(
                same_time,
                receiver,
                census,
                usage,
        ...
        current = json.loads(
            self.paths.estate_opportunity_current.read_text()
        )
        self.assertEqual(
            current["source_usage_receipt_sha256"],
            usage_result["receipt_sha256"],
        )
        altered = dict(opportunity)
        altered["source_usage_snapshot_sha256"] = "sha256:" + "3" * 64
        altered_snapshot = {
            key: value
        ...
        altered["snapshot_sha256"] = runtime_module.digest(altered_snapshot)
        with self.assertRaisesRegex(RuntimeFailure, "estate-opportunity-invalid"):
            core.record_estate_opportunity(
                altered,
                receiver,
                census,
        ...
        unknown_category["snapshot_sha256"] = runtime_module.digest(
            {
                key: value
        ...
        inconsistent_total["snapshot_sha256"] = runtime_module.digest(
            {
                key: value
        ...
        boolean_version["snapshot_sha256"] = runtime_module.digest(
            {
                key: value
        ...
        zero_distinct["categories"][0]["distinct_day_count"] = 0
        zero_distinct["snapshot_sha256"] = runtime_module.digest(
            {
                key: value
        ...
        invalid_failure_id["snapshot_sha256"] = runtime_module.digest(
            {
                key: value
        ...
        invalid_possible_category["snapshot_sha256"] = runtime_module.digest(
            {
                key: value
        ...
        naive_window["snapshot_sha256"] = runtime_module.digest(
            {
                key: value
        ...
        naive_category_time = copy.deepcopy(opportunity)
        naive_category_time["categories"][0]["last_observed_at"] = (
            "2026-08-17T18:00:00"
        )
        naive_category_time["snapshot_sha256"] = runtime_module.digest(
            {
                key: value
                for key, value in naive_category_time.items()
                if key != "snapshot_sha256"
            }
        )
        invalid_values.append(naive_category_time)
        for invalid in invalid_values:
            with self.subTest(invalid=invalid):
                with self.assertRaisesRegex(
                    RuntimeFailure,
                    "estate-opportunity-invalid",
                ):
                    core.record_estate_opportunity(
                        invalid,
                        receiver,
    ...
    def test_opportunity_contract_matches_origin_collector(self) -> None:
        path = SCRIPT_DIR / "dreaming-estate.py"
        spec = importlib.util.spec_from_file_location(
            "dreaming_estate_contract_test",
            path,
        )
        ...
        self.assertEqual(
            runtime_module.TASK_OPPORTUNITY_TAXONOMY_ID,
            estate.task_opportunity_taxonomy_id(),
        )
        self.assertEqual(
            runtime_module.TASK_OPPORTUNITY_CLASSIFIER_ID,
            estate.task_opportunity_classifier_id(),
        )
        self.assertEqual(
            runtime_module.TASK_OPPORTUNITY_CATEGORY_IDS,
            {
                item["category_id"]
                for item in estate.TASK_OPPORTUNITY_TAXONOMY["categories"]
            },
        )
        self.assertEqual(
            runtime_module.MAX_TASK_OPPORTUNITY_RECEIPT_EPISODES,
            estate.MAX_OPPORTUNITY_RECEIPT_EPISODES,
        )
