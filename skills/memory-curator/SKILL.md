...
> **Paths note:** Run this skill's scripts from
> `$DREAMING_REPO_ROOT/skills/memory-curator`. Its authenticated browser helper
> resolves only from
> `$DREAMING_SHARED_SKILLS_ROOT/skills/authenticated-browse`; installed-plugin
> scanning is intentionally unsupported.
...
1. **Extract**:
   `mem-extract.sh -o "$SKILLS_STATE_DIR/memory-curator/scratch/mem.json"`.
   If exit 5, tell the user to
   re-auth (`authenticated-browse auth default <memory-url>`) and stop.
2. **Filter new**:
   `cat "$SKILLS_STATE_DIR/memory-curator/scratch/mem.json" | mem-ledger.sh filter-new`
   to skip
...
3. **Categorize** each memory: `roll` (durable, belongs in a skill), `dup`
   ...
   Map each `roll` to a target skill
   (`ls "$SKILLS_REPO_ROOT/skills" "$SKILLS_LOCAL_ROOT"`).
...
5. **Review before committing.** Rolling grows skills, so hold each edited
   skill to `writing-great-skills` from `$DREAMING_SHARED_SKILLS_ROOT` and run
   `dual-review` from that same verified root on the diff. Rolls are where
   sediment accumulates and no human reads the diff.
...
8. **Delete**:
   `mem-delete.sh -f "$SKILLS_STATE_DIR/memory-curator/scratch/delbodies.json"`.
