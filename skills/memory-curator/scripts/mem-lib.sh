...
mem_locate_pw() {
  local config requested_repo requested_shared receipt
  ...
  if [ -f "$config" ]; then
    # Generated by $DREAMING_REPO_ROOT/scripts/dreaming-deps.py.
    # shellcheck disable=SC1090
    source "$config"
  fi
  DREAMING_REPO_ROOT="${requested_repo:-${DREAMING_REPO_ROOT:-}}"
  DREAMING_SHARED_SKILLS_ROOT="${requested_shared:-${DREAMING_SHARED_SKILLS_ROOT:-}}"
  [ -n "$DREAMING_REPO_ROOT" ] && [ -n "$DREAMING_SHARED_SKILLS_ROOT" ] || return 1
  receipt="${DREAMING_RECEIPT_FILE:-$DREAMING_REPO_ROOT/scripts/shared-deps-receipt.json}"
  DREAMING_RECEIPT_FILE="$receipt" \
    "$DREAMING_REPO_ROOT/scripts/dreaming-deps.py" verify \
      "$DREAMING_SHARED_SKILLS_ROOT" >/dev/null 2>&1 || return 1
  printf '%s\n' \
    "$DREAMING_SHARED_SKILLS_ROOT/skills/authenticated-browse/scripts/pw-session.sh"
}
