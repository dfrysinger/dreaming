...
agent's working context verbatim (path-swapped to the managed public/local
roots, `skill_manage` tool name → `/skill-manage` skill).
...
>
> 1. DO NOT touch skills outside `$SKILLS_REPO_ROOT/skills/` and `$SKILLS_LOCAL_ROOT/`. Dreaming-owned and shared-bundle skills are dependencies, not curation targets; other plugins are off-limits.
