...
> **Paths note:** Run Dreaming-owned helpers from `$DREAMING_REPO_ROOT`.
> Shared authoring guidance resolves from `$DREAMING_SHARED_SKILLS_ROOT`.
> `$SKILLS_REPO_ROOT` remains the optional public catalog.
...
7. **Name is short, distinct, and reachable in 3-4 keystrokes.** Prefer one word (`scout`, `explain`, `prototype`); use two when the second word earns its place by disambiguating (`dual-review`, `self-compact`, `visual-proof`). Avoid naming a skill after a CLI command it drives — `/compact` and `/autopilot` are typed by the user, and a skill sharing that name reads as the command.
   ...
   ```bash
   "$DREAMING_REPO_ROOT/skills/skill-manage/scripts/check-name-prefix.sh" <slug>
   ```
...
2. **Pre-flight collision check across BOTH roots:**
   ...
   Also run
   `"$DREAMING_REPO_ROOT/skills/skill-review/scripts/check-tombstone.sh" <name>`
...
7. **Mark provenance** (only when this skill was created autonomously, not by hand):
   ```bash
   "$DREAMING_REPO_ROOT/skills/skill-review/scripts/mark-agent-created.sh" \
...
2. **Register the skill in `.claude-plugin/plugin.json`** (REQUIRED — skills
   ...
   ```bash
   "$DREAMING_REPO_ROOT/skills/skill-manage/scripts/registry.sh" register <name>
   ```
...
use `"$DREAMING_REPO_ROOT/skills/skill-manage/scripts/promote-skill.sh" <name>`
instead
...
- **Vendoring third-party skills.** When importing externally-authored skills,
  ...
  `NOTICE.md` (see
  `$DREAMING_REPO_ROOT/skills/skill-review/references/NOTICE.md` for the
  existing example).
